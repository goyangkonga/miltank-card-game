...
class GachaScene {
  ...
  _build() {
    ...
    <div id="gs-stage" style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:20px 16px;">
      <div id="gs-progress" style="font-size:12px;color:var(--text-sub);font-weight:500;"></div>
      <div id="gs-cow-wrap" style="position:relative;cursor:pointer;user-select:none;" onclick="gachaScene._squeeze(event)">
        <div id="gs-cow-emoji" style="font-size:80px;line-height:1;animation:cowBob 1.8s ease-in-out infinite;">🐄</div>
      ...
      <button id="gs-next" class="btn btn-primary btn-full" style="display:none;"></button>
    </div>
    <div id="gs-multi" style="display:none;padding:16px;">
      ...
      <button class="btn btn-primary btn-full" style="margin-top:20px;" onclick="gachaScene.onDone && gachaScene.onDone()">농장으로 이동</button>
    </div>`;
  }
  ...
  _squeeze(e) {
    ...
    drop.textContent = '💧';
    drop.style.cssText = `position:absolute;font-size:14px;pointer-events:none;left:${30+Math.random()*40}%;top:${30+Math.random()*30}%;animation:squirt .5s ease forwards;`;
  ...
  _showMultiResult() {
    ...
    this.pendingCards.forEach(card => {
      ...
      item.innerHTML += `<div style="font-size:10px;font-weight:500;color:${col.text};text-align:center;">${card.name}</div><span style="font-size:9px;padding:1px 6px;border-radius:20px;background:${col.bg};color:${col.text};border:1px solid ${col.border};">${card.grade}</span>`;
